...
class Feature(object):
    ...
    #x为输入，max_len为这个输入的最长长度，pad_idx是什么？是指用什么东西代表pad吗？
    @classmethod
    def _add_pad_and_gene_mask(cls,x,max_len,pad_idx = 0,return_mask = True):
        pad_len = max_len - len(x)#需要把输入增添到最大长度
        assert pad_len >= 0

        if return_mask:#如果需要mask，就返回用0填充的
            mask = [1] * len(x)
            mask.extend([0] * pad_len)
            assert len(mask) == max_len

        x.extend([pad_idx] * pad_len)#pad_idx还真代表了填充物
...
def content_word_to_idx_with_oov(content_list,vocab):
    ...
    for word in content_list:
        #如果这个字在字表中，返回它自身的索引，如果不在，返回未知词的索引
        idx = vocab.word_2_idx(word)
        if vocab.unk_idx == idx:
            if word not in oovs:#并添加到超词表中
                oovs.append(word)
            oov_idx = oovs.index(word)#在超词表中搜索这个词的索引
            indexes.append(vocab.get_vob_size() + oov_idx)#把它添加到超词表之后的索引，就是在超词表已有的索引上累加
        else:
            indexes.append(idx)
    return indexes,oovs#返回带有本身词表和超词表的索引以及超词


def target_target_idx_with_oov(title_list, vocab , oovs):
    target_with_oov = []
    for word in title_list[1:]:#为什么不要第一个?因为第一个是开始符号
        idx = vocab.word_2_idx(word)
        #和文本操作的是一样的
        if vocab.unk_idx == idx:
            if word in oovs:
