                ...
                for sample in data:
                    ...
                    if (read_sample_num == train_terminate) or \
                        ...
                        with open(filename, 'w', encoding='utf-8') as save_f:
                            json.dump(save_dict, save_f,ensure_ascii=False)
                            save_f.close()
...
def main():
    ...
    # F:\data\zh\news
    parser.add_argument("--original_data_dir",default="E:\\工作\\毕业论文\\LCSTS数据集\\dataTest",type = str,required=False,
                        help="包含文件lcsts.csv的文件夹路径")
    # F:\data\zh\tokenized
    parser.add_argument("--tokenized_dir", default="E:\\工作\\毕业论文\\LCSTS数据集\\token_single", type=str, required=False,
                        help="分词后文件所存储的文件夹")

    parser.add_argument("--split_data_dir",default = "sub",type=str,
                        help="切分后的文件存储的文件夹(源json文件太大了)")

    parser.add_argument("--sub_file_sample_num",default=10000,type=int,
                        help="每个子文件存储的样本数量")
    ...
    parser.add_argument("--train_sample_num",default=8000,type=int,
                        help="训练集样本的数量")
    parser.add_argument("--dev_sample_num",default=1000,type=int,
                        help="验证集样本的数量")
    parser.add_argument("--test_sample_num",default=1000,type=int,
                        help="测试集样本的数量")
